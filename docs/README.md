...
- `docs/構成/発表内容_研究室内中間報告.md` — 10分発表の内容と構成
- `docs/構成/発表_禁止表現チェックリスト.md` — 撤回値・過大主張の混入防止
- `docs/構成/発表_想定質疑.md` — 質疑への回答方針と測定限界

`docs/構成/` 配下のファイル名が`発表`で始まる発表関連ファイルは、発表準備のための必要文書として**ファイル削減の対象外**とする。発表終了後も、次回発表・最終審査で再利用する可能性があるため、終了済みという理由だけでは削除しない。
...
- 発表関連ファイルは `docs/構成/` に残し、削減対象外とする。
