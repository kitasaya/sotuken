...
日本の交通法規に準拠した自転車向けナビゲーションシステム。
経路レベルで排除する一方通行の逆走と、走行時に案内する二段階右折・一時停止・踏切を扱う。
...
2. **preparing モード**（出発前）：
   ...
   - 案内地点（一時停止・踏切）に紫のマーカーが表示される
   - 下部パネルで違反一覧・案内一覧・推奨情報を確認できる。
     「走行時の案内」セクションのカードをタップすると地図がその地点に移動し、
     地図のマーカーをタップすると該当カードまでスクロールする
   - 件数は「⚠️ 違反候補 N 件」と「🔔 走行時の案内 M 件」を別行で表示する
     （案内は違反ではないため合算しない）
3. 「走行開始」ボタンまたは GPS 速度 5km/h 以上で **riding モード** に自動切り替え：
   - 次の曲がり角の矢印と距離のみを大きく表示
   - 二段階右折・一時停止・踏切が近づくとバナーと手順パネルが表示される
