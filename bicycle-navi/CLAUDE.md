...
### 進捗スナップショット（2026-06-07 セッション終了時点）
...
| Q2：進行方向照合のロバスト化 | **実装完了・計測待ち** | ジオメトリクリップ + 20m 短路スキップ + 多数決。PERFORMANCE.md Q2 セクション参照 |

### 再開時の最初のアクション

Q2 の実装は完了。Masaya さんによる実機計測が残っている：
...
計測結果（違反総数・confidence=1.0 件数・confidence=0.7 件数・リルート数）を
`docs/PERFORMANCE.md` の Q2 セクションの表に記入する。

計測後の選択肢：

1. **性能向上フェーズを完了として UI フェーズ（U3/U4）に入る**：riding モードの
   フルスクリーン化（U3）、スマホ実機テスト（U4）。CLAUDE.md を UI フェーズ版に
   書き直す。
2. **論文執筆フェーズに入る**：実装を停止し、CHANGELOG.md / PERFORMANCE.md の
   数値を論文セクションに反映する作業に切り替える。

判断材料：
- P1+P2 で速度目標は達成済み（cold 2.4s、warm 0.0s）
- Q2 で違反件数の変動があった場合は Q1 と同様の考察になる可能性あり
- Q2 の「偽陽性削減」は confidence 分布の改善として論文に記述可能
