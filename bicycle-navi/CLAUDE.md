...
### 進捗スナップショット（2026-06-05 セッション終了時点）
...
| Q2：進行方向照合のロバスト化 | **未着手** | 下記「再開時の最初のアクション」参照 |

### 再開時の最初のアクション

次回 Claude Code を起動したとき、Masaya さんから明確な指示がなければ
**まずユーザーに以下のいずれかを選んでもらう**：

1. **Q2 を実施する**：進行方向照合のロバスト化（短い way の処理、カーブ区間の
   ベクトル分割、`confidence=1.0` 比率の向上）。ただし Q1 の経験から、
   「ヒューリスティック法規検出を触ると違反件数が変動しうる」点に注意。
   実施前に「どの指標を改善とみなすか」を決めるべき。
2. **性能向上フェーズを終了して UI フェーズ（U3/U4）に戻る**：riding モードの
   フルスクリーン化、スマホ実機テスト。CLAUDE.md を UI フェーズ版に書き直す。
3. **性能向上フェーズを終了して論文執筆フェーズに入る**：実装は一旦停止し、
   docs/CHANGELOG.md と docs/PERFORMANCE.md の数値を論文セクションに反映
   する作業に切り替える。

選択肢の判断材料：
- P1+P2 で当初目標（p50 < 2s, p95 < 4s）は cold ですら 2.4 秒で十分達成済み
- Q1 ロールバックで「品質改善は構造的に難しい」というネガティブだが論文の
  考察になる結果が得られている
- Q2 で本当に有利な数字が出るかは未知数（少なくとも違反件数指標では Q1 と
  同じ問題が出る可能性あり）
