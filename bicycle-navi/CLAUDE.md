...
性能改善ログ詳細は `docs/PERFORMANCE.md` を参照。

### 進捗スナップショット（2026-06-12 セッション終了時点）

| タスク | 状態 | メモ |
|---|---|---|
| P1：Overpass キャッシュ + UA + circuit breaker | **完了** | cold 27.9s → 2.4s、warm 0.0s |
| Q1：自転車インフラ優先 custom_model | **試行→ロールバック** | 違反検出が +13 件になる構造的問題。詳細は CHANGELOG/PERFORMANCE.md |
| P2：共有 httpx クライアント + コネクションプール | **完了** | lifespan で起動/終了管理 |
| Q2：進行方向照合のロバスト化 | **完了** | クリップ + 多数決。confidence=1.0 比率 100% 維持（Pre-Q2 と同水準）。詳細は PERFORMANCE.md |
| U3：riding モードのフルスクリーン化 + heading-up | **完了** | ヘッダー削除・オーバーレイ化・地図 CSS 回転（scale1.5）・コンパス追加 |
| U4：スマホ実機テスト | **進行中** | UI 確認済み。GPS 動作確認済み。音声・heading-up・自動切り替えは屋外テスト残り |

### 開発環境（HTTPS）セットアップ済み

mkcert でローカル HTTPS 証明書を設定済み（2026-06-12）。
- 証明書: `frontend/localhost+1.pem`（.gitignore 対象）
- vite.config.js で `https: { key, cert }` を設定済み
- iPhone に rootCA をインストール済み（設定 → 一般 → 情報 → 証明書信頼設定）
- 起動: `npm run dev -- --host` → `https://192.168.11.5:5173` でスマホアクセス可

### 再開時の最初のアクション

**U4 残りチェック（屋外・実走行）：**
1. **音声案内** — riding モードで「音声ON」後に「次の案内へ」で音声が出るか（室内でも確認可）
2. **Heading-up** — 歩きながら riding モードの地図が進行方向に回転するか・右上「↑N」コンパスが出るか
3. **GPS自動モード切り替え** — 5km/h 以上で riding モードに自動切り替え・停車 5秒で preparing に戻るか

U4 完了後は論文執筆フェーズへ移行予定。

### 性能向上フェーズの成果まとめ（論文記述候補）

| タスク | 結果 | 論文への反映 |
|---|---|---|
| P1 | cold 27.9s → 2.4s（-91%）、warm 0.0s | レスポンス時間改善の主軸 |
| P2 | TCP/TLS 共有化 | P1 の補完（定量値は未計測） |
| Q1 | ロールバック | 「cycleway 優遇とヒューリスティック違反検出の相性問題」として考察 |
| Q2 | confidence=1.0 比率 100% 維持、偽陽性なし | 「改善余地が既にない水準に達していた」と記述 |
