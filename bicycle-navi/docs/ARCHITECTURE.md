...
- 案内地点（一時停止・踏切）の接近時はバナー＋手順パネル＋音声で案内
- 次の交差点までの距離を大きな数字で表示
- 違反交差点では矢印に警告マークを重ねる
- 地図表示は最小限（現在地 + 数十メートル）
...
- ルート全体を地図上に表示
- 違反箇所をピン表示（赤：確実／黄：要確認）
- 案内地点（一時停止・踏切）を紫のピンで表示。違反より小さい半径にして優先度の差を示す
- 二段階右折の手順をステップ形式で表示
- 違反一覧パネルで violations / recommendations を全件確認可能
- 違反一覧とは別セクションの「走行時の案内」で guidance を全件確認可能。
  件数表示も「違反候補 N 件」と「走行時の案内 M 件」を別行に分け、合算しない
...
| 一時停止 | `highway=stop`（node） | **違反ではなく案内**。経路が通過した区間上の node のみ抽出し、`guidance` 配列で返却 |
| 踏切 | `railway=level_crossing`（node） | **違反ではなく案内**。同上 |

一時停止・踏切は「二層の責任分界」の**第2層（走行ガイダンス）** に属する。
経路が決まっても違反の有無は決まらず、利用者の走行方法に依存するためである
（二段階右折と同じ扱い）。したがって `violations` には入れず、独立した
`guidance` 配列で返す。**`violations` の件数定義には一切含めない。**
