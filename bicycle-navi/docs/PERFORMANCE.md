...
## バッチ21：閉ループgeometry切出しの回帰（2026-08-30）

- 閉ループwayでは重複終端を除いた2候補弧を作り、実ルート座標列への適合距離で選択する。
- way 28413951の旧7/7逆向き判定は順走へ訂正した。
- 非閉ループ16ケースは旧実装と返却geometryが完全一致し、R2全379点の判定結果も不一致0件だった。
- 固定15 ODの標準bike／現行方式は全件同一で、距離差0.0mだった。追加areasは未発動のため性能向上の実証値ではない。
