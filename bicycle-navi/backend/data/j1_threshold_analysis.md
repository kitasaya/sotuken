...
|---|---:|---:|
| 一時停止（3m以内・方向適用） | 5点 ※1 | 20点 |
| 踏切（幾何交差・クラスタ25m） | 7箇所 | 2箇所 |
| 検出ゼロのペア（一時停止） | 11/15 | 5/15 |
...
**way_id を正とすると 3m 基準の再現率は 33/40 = 82.5%、取りこぼしは7件。**
3m以内と判定されたノードは全件が経路way上にあり、誤検出は0件だった。
一方 Google 側には way_id が無いためこの検算ができない。3m 基準は自システム側で
82.5%、Google 側は測定不能、という非対称のまま両者の数字が出ている。
...
除去できない一時停止20点・踏切2箇所が残っている。** 事前排除の対象にできる
