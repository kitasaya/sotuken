...
⚠️ **2026-07-30：このフィルタは不採用と決定した。** 本スクリプトはその判断の
根拠となった実測を再現するために残してある。`services/` へ実装を移してはならない。
判断の内容と根拠は RESEARCH.md 21.13節、実測は data/dryrun_nonroad_filter.md を参照。
...
def build_markdown(rows: list[dict], labels_run: list[str], skipped: list[str]) -> str:
    ...
    a("## 決定（2026-07-30）")
    a("")
    a("> ⚠️ **非車道除外フィルタは不採用と決定した。** 本レポートはその判断の根拠となった実測である。")
    a("> C群3点は保持されるため事前に定めた不採用条件そのものには該当しないが、")
    a("> **B群への効果がゼロ**・**判定変化4点がすべて新規出現**・")
    a("> **切替37点の垂線距離が中央値 +3.97m / 最大 +125.4m 悪化**する、という3点により不採用とした。")
    a("> 判断の全文は RESEARCH.md 21.13節を参照。")
    a("")
    a("以下はドライラン実行時点の自動生成結果であり、上記の決定を反映していない"
      "（スクリプトは採否が未定の前提で結論文を生成する）。")
    a("")
