...
> 各判定点について、OSM wayを構成する有限線分への最小距離をway単位で算出し、距離が最小のwayを対応先とした。候補の曖昧性は、異なるway IDを持つ第1候補と第2候補の距離差 `match_margin_m` で表し、2m未満を判定不能とした。検証対象379点中188点（49.6%）がこの条件に該当し、同一wayの重複によるものは0点だった。
