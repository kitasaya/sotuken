...
`match_margin_m` は、**互いに異なるOSM way IDを持つ第1候補と第2候補の有限線分最小距離の差**である。同じwayがUnionクエリ結果に重複しても、way IDで集約されるため第1・第2候補を同じwayが占めない。これはrank1と次点wayの幾何的近接度であり、対応づけの正しさに対する信頼度ではない。
