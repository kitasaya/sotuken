...
このscoreは本研究の `match_margin_m` と同じ式ではないが、「候補対応の信頼度を数値化し、閾値で受理・保留を分ける」という設計思想は近い。
...
> 本研究では上位2候補の距離差を `match_margin_m` として保留判定に用いる。この指標と2m閾値は先行研究の定式を転用したものではなく、本研究の入力座標精度と感度分析に基づく運用指標である。
